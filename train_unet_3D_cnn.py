...
class UNet3DCNN(nn.Module):
    ...
    """
    def __init__(self, in_ch, out_ch, base_ch=32, bottleneck_ch=64, kernel=3, seq_len_out=1):
        super().__init__()
        ...
        # Bottleneck
        self.bottleneck = DoubleConv3D(base_ch*4, bottleneck_ch, kernel)
        # Decoder
        self.up1 = Up3D(bottleneck_ch, base_ch*2, base_ch*2, kernel)
...
def train_radar_model(
    ...
    *,
    ...
    model     = UNet3DCNN(in_ch=C, out_ch=C, base_ch=hidden_dims[0], bottleneck_ch=hidden_dims[1], kernel=kernel_size, seq_len_out=seq_len_out).to(device)
...
def predict_validation_set(
    ...
    *,
    ...
    model = UNet3DCNN(in_ch=cube.shape[1], out_ch=cube.shape[1], base_ch=hidden_dims[0], bottleneck_ch=hidden_dims[1], kernel=kernel_size, seq_len_out=seq_len_out)
    ...
    with torch.no_grad():
        for xb,yb in dl:
            ...
            if yb.shape[2] == 1:
                yb = yb.squeeze(2)
            preds.append(out_n.cpu().numpy()*(maxv+eps))
            gts.append(yb.cpu().numpy()*(maxv+eps))
