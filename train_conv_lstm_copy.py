...
def masked_mse_loss(pred, target, threshold=0.40):
    """
    Masked MSE that only computes MSE for target values above a given threshold (normalized reflectivity).
    Parameters:
    -----------
    pred: torch.Tensor
        Predicted values (normalized to [0,1])
    target: torch.Tensor
        Ground truth values (normalized to [0,1])
    threshold: float
        Only include pixels where target > threshold in the MSE calculation.
    """
    mask = target > threshold
    if mask.sum() == 0:
        # If no pixels above threshold, fall back to standard MSE
...
def train_radar_model(
    ...
    *,
    ...
    loss_name : str, optional
        Loss function to use; either 'mse', 'weighted_mse', or 'masked_mse'.
    ...
    elif loss_name == "masked_mse":
        criterion = lambda pred, tgt: masked_mse_loss(
            pred, tgt,
            threshold=loss_weight_thresh
        )
    else:
...
def predict_validation_set(
    ...
    *,
    ...
    patch_frac : float, optional
        Minimum fraction of pixels in patch above threshold (default: 0.15).
    use_patches : bool, optional
        Whether to use patch-based validation (default: False).
...
if __name__ == "__main__":
    ...
    parser.add_argument("--loss_name", type=str, default="mse", help="Loss function: mse, weighted_mse, or masked_mse")
